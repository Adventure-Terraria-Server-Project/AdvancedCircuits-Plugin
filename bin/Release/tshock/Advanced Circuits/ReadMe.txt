...
Plugin Cooperation
--------------------------------------------------------------
This plugin will cooperate with Sign Commands (by Scavenger)
if installed. Wired signs with a sign command can then be 
triggered by Advanced Circuits to have the commands executed
for the player who triggered the circuit.
Note that this will not work when Infinite Signs is installed.

If Protector is installed, then the "/ac switch" command will 
not work on protected objects.


Known Problems
--------------------------------------------------------------
In order to fully override Terraria's wiring mechanism you
will need to replace your TerrariaServer.exe with the custom
executable provided with this plugin.
